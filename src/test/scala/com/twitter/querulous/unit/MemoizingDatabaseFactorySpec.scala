...
class MemoizingDatabaseFactorySpec extends Specification with JMocker {
  ...
  "MemoizingDatabaseFactory" should {
    "apply" in {
      ...
      expect {
        one(databaseFactory).apply(hosts, "bar", username, password, DatabaseFactory.defaultUrlOptions) willReturn database1
        one(databaseFactory).apply(hosts, "baz", username, password, DatabaseFactory.defaultUrlOptions) willReturn database2
      }
